...
| ICICI Prudential Multi-Asset Fund | enriched, **fails 8 gates** | PASS — worst-5Y +0.040, 5Y Sortino **1.92** (best measured of any candidate) | Gate failures are mechanical (no alpha/beta/R²/category columns published anywhere — cross-confirmed against a second data source that also shows Beta "–"), and the fund itself is excellent: Sharpe 1.10 / StdDev 8.85 (both cross-validated to the decimal), ₹84k Cr AUM, 0.52% TER, worst drawdown −23.77% (COVID) recovered in 3 months, +6% in the 1Y the Nifty lost 5.4%. **Ruled out anyway on measured geometry (2026-07-09): its equity book overlaps the Parag Parikh core by 22.15% min-weight (lower bound — 36.6% of its book is unseen; worst case 73%) vs the 10% cap** — the common names are the same HDFC Bank / ICICI / Axis Bank / Infosys / ITC block the core already holds. Its genuinely uncorrelated part is the ~35% gold/debt sleeve; a 20% allocation buys only ~7% of that at portfolio level while doubling up the same large-caps |
